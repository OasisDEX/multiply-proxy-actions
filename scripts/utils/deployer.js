...
const deployWithResend = (contractName, signer, action, exGasPrice, nonce, ...args) =>
  new Promise((resolve) => {
    getGasPrice(exGasPrice).then((gasPrice) => {
      let deployPromise = deploy(contractName, signer, action, gasPrice, nonce, ...args)
      const timeoutId = setTimeout(
        () => resolve(deployWithResend(contractName, 'Resending', gasPrice, nonce, ...args)),
        parseFloat(process.env.TIMEOUT_MINUTES) * 60 * 1000,
      )
