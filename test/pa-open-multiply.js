const { deploySystem, TEN, getOraclePrice, dsproxyExecuteAction, getLastCDP, getVaultInfo, balanceOf, MAINNET_ADRESSES } = require('../test/common/mcd-deployment-utils');
const { getMarketPrice, exchangeFromDAI, exchangeToDAI, getCurrentBlockNumber } = require('../test/common/http_apis');
const { calculateParamsIncreaseMP, calculateParamsDecreaseMP, amountToWei, prepareMultiplyParameters, addressRegistryFactory } = require('../test/common/params-calculation-utils');
...
*/
    ...
    it("it should send fee to beneficiary", async function () {
      ...
      console.log("FEE INFO: amount from event - ", feeAmount.toFixed(0), "amount computed from input data - ", expected.toFixed(0),
        "balance difference - ", balanceDifference, "Fee recipient:", ADDRESS_REGISTRY.feeRecepient, scRecipient);
