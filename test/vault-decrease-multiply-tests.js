const {deploySystem, FEE,FEE_BASE,TEN,  getOraclePrice, dsproxyExecuteAction, getLastCDP, getVaultInfo, MAINNET_ADRESSES, addressRegistryFactory} = require('./common/mcd-deployment-utils');
const {getMarketPrice, exchangeFromDAI, exchangeToDAI, getCurrentBlockNumber} = require('./common/http_apis');
const {calculateParamsIncreaseMP, calculateParamsDecreaseMP, amountToWei, 
  prepareBasicParams,packMPAParams, convertToBigNumber, ensureWeiFormat} = require('./common/params-calculation-utils');
...
var testVaults = [
  {
    ...
    _1inchPayload: {to:"0x111111111117dc0aa78b770fa6a738034120c302",
    data:"0x111111111117dc0aa78b770fa6a738034120c302"},//irrelevant, for mock exchange just for encoding validation passing
    desiredCDPState:{
...
const restore = function(el){ // keeps same reference, eg. in a table
  if(el.__backup){
 ...
 async function runTestCase(testCase,testParam){
    return new Promise((res,rej)=>{ //to run several in runner, one after another
      ...
      const calculateExchangeData = async function(paramCalcFunc,data, exchangeQuery){
          let [requiredDebt, toBorrowCollateralAmount] = paramCalcFunc(oraclePrice, marketPrice, OUR_FEE, AAVE_FEE,
            (new BigNumber(data.desiredCDPState.providedCollateral)).plus(new BigNumber(data.currentColl)), 
        ...
        // data._1inchPayload = await exchangeQuery(requiredDebt,data);
          ...
          if(requiredDebt<0 || toBorrowCollateralAmount <0){
            console.log("calculateExchangeData, strange values",requiredDebt.toFixed(0),toBorrowCollateralAmount.toFixed(0),data);
          }
      ...
      const calculateRequiredDebt = function(operation,existingCDP, desiredCDPState, oraclePrice, marketPrice,slippage, debug = false, daiAmount = 0,withdrawColl = 0 ){
        let debtDelta;
      ...
      const updateLastCDPInfo = async function(data,signer, provider, userProxyAddr){
        ...
        var vaultInfo = await getVaultInfo(deployedContracts.mcdViewInstance, lastCDP.id, lastCDP.ilk);
        data.existingCDP = {
          coll:vaultInfo.coll,
          debt:vaultInfo.debt,
          id:lastCDP.id,
      ...
      this.afterEach(async function(){
        ...
        this.beforeAll(async function(){
          ...
          ADDRESS_REGISTRY = addressRegistryFactory(deployedContracts.multiplyProxyActionsInstance.address,deployedContracts.exchangeInstance.address);
          
          oraclePrice = await getOraclePrice(provider);
          if(testParam.useMockExchange){
            marketPrice = oraclePrice.multipliedBy(1-testParam.oraclePriceDivergence);
            await deployedContracts.exchangeInstance.setPrice(marketPrice.multipliedBy(TEN.pow(18)).toFixed(0));
        ...
        describe(`opening Multiply Vault with collateralisation ratio of ${testCase.desiredCDPState.desiredCollRatio}`, async function(){
          ...
          this.beforeAll(async function(){
            ...
            let status;
            [status,txResult] =  await dsproxyExecuteAction(deployedContracts.multiplyProxyActionsInstance,
          ...
          it(`it should open vault with collateralisation Ratio of ${testCase.desiredCDPState.desiredCollRatio}`, async function(){
            precision = 10;
            var actualRatio = div(mul(testCase.existingCDP.coll,oraclePrice),testCase.existingCDP.debt);
            ...
            console.warn(`\x1b[33m Vault params:Collateral ${testCase.existingCDP.coll}, Debt ${testCase.existingCDP.debt} \x1b[0m`);
            expect(actualRatio.toNumber()).to.be.greaterThanOrEqual(testCase.desiredCDPState.desiredCollRatio);//final collaterallisation value equal to at least desired
          ...
          it(`it should flash loan correct amount of DAI`, async function(){
            ...
            var flDataEvent = allEvents.filter(x=>x.firstTopic === "0x9c6641b21946115d10f3f55df9bec5752ec06d40dc9250b1cc6560549764600e")[0];
          ...
          it("it should send fee to beneficiary", async function(){
            ...
            var feePaidEvents = allEvents.filter(x=>x.firstTopic === "0x69e27f80547602d16208b028c44d20f25956e1fb7d0f51d62aa02f392426f371");
            ...
            endBalance = await  deployedContracts.daiTokenInstance.balanceOf(ADDRESS_REGISTRY.feeRecepient);
            var balanceDifference = endBalance.sub(startBalance).toString();
            expect(feeAmount.toNumber()).to.be.greaterThanOrEqual(expected.toNumber()*(1-precision/100));//due to possible rounding errors
          ...
          describe(`Increasing CollateralisationRatio from ${testCase.desiredCDPState.desiredCollRatio} to ${testParam.desiredCollRatio} without withdrawal`, async function(){
            ...
            it(`should increase CollateralisationRatio to ${testParam.desiredCollRatio} `,async function(){
              ...
              const collRatio = div(mul(testCase.existingCDP.coll,oraclePrice),testCase.existingCDP.debt).toNumber();
              expect(collRatio).to.be.greaterThan(testParam.desiredCollRatio*(1-negativeMargin/100));//to accout for rounding errors
              expect(collRatio).to.be.lessThanOrEqual(testParam.desiredCollRatio*(1+positiveMargin/100));//due to slippage smaller than maximum we might end up with higher coll ratio
              console.warn(`\x1b[33m${positiveMargin}% positive and ${negativeMargin}% negative margin for collateralisation ratio applied actual Value ${collRatio}\x1b[0m`)
            })
          ...
          describe(`Increasing CollateralisationRatio to ${testParam.desiredCollRatioDAI} with DAI withdrawal`, async function(){
            ...
            this.beforeAll(async function(){
              ...
              [status,inTxResult] = await dsproxyExecuteAction(deployedContracts.multiplyProxyActionsInstance,
                  deployedContracts.dsProxyInstance, 
                  primarySignerAddress, 
                  'decreaseMultipleWithdrawDai', params);        
              if(!status){
                    restoreSnapshot.lock = true;
                    throw "Tx failed";
                    throw inTxResult;
            ...
            it(`should increase CollateralisationRatio to ${testParam.desiredCollRatioDAI}`,async function(){
              ...
              var positiveMargin = 5;
              const collRatio = div(mul(testCase.existingCDP.coll,oraclePrice),testCase.existingCDP.debt).toNumber();
              expect(collRatio).to.be.greaterThan(testParam.desiredCollRatioDAI*(1-negativeMargin/100));//to accout for rounding errors
              expect(collRatio).to.be.lessThanOrEqual(testParam.desiredCollRatioDAI*(1+positiveMargin/100));//due to slippage smaller than maximum we might end up with higher coll ratio
              console.warn(`\x1b[33m${positiveMargin}% positive and ${negativeMargin}% negative margin for collateralisation ratio applied actual Value ${collRatio}\x1b[0m`)
            })
            it(`should change primaryAddress DAI balance by exacly ${testParam.desiredDAI} DAI`,async function(){
              
              var balanceAfter = await  deployedContracts.daiTokenInstance.balanceOf(await primarySigner.getAddress());
          ...
          describe(`Increasing CollateralisationRatio to ${testParam.desiredCollRatioETH} with Collateral withdrawal ${testParam.desiredETH} ETH`, async function(){
            ...
            this.beforeAll(async function(){
              ...
              [status,inTxResult] = await dsproxyExecuteAction(deployedContracts.multiplyProxyActionsInstance,
                  deployedContracts.dsProxyInstance, 
                  primarySignerAddress, 
                  'decreaseMultipleWithdrawCollateral', params);   
              if(!status){
                restoreSnapshot.lock = true;
                throw "Tx failed";
                throw inTxResult;
            ...
            it(`should not change primaryAddress DAI balance`, async function(){
              ...
              var daiAfter = await  deployedContracts.daiTokenInstance.balanceOf(await primarySigner.getAddress());
          ...
          describe("Close Vault, withdraw collateral", async function(){
            ...
            this.beforeAll(async function(){
              ...
              exchangeData.toTokenAmount    = ensureWeiFormat(mul(testCase.existingCDP.debt,add(add(1,AAVE_FEE),OUR_FEE)));
              exchangeData.minToTokenAmount = ensureWeiFormat(mul(testCase.existingCDP.debt,add(add(1,AAVE_FEE),OUR_FEE)));
              exchangeData.fromTokenAmount  = ensureWeiFormat(div(exchangeData.toTokenAmount,mul(marketPrice,sub(1,testParam.slippage))));
            ...
            it("should send to user all collateral in a vault minus debt",async function(){
              ...
              console.log("balances",closingVaultInfo.coll.toFixed(4),closedVaultInfo.coll.toFixed(4),closingVaultInfo.debt.toFixed(4),closedVaultInfo.debt.toFixed(4));
              var expectedReturnedAmount = sub(closingVaultInfo.coll,
                  div(
                    mul(closingVaultInfo.debt,
                      add(
                        add(
                          add(
                            1,
            ...
            it("should send to user no DAI",async function(){
              expect(convertToBigNumber(daiBefore).toFixed(0)).to.be.equal(convertToBigNumber(daiAfter).toFixed(0));
            })
            it("ProxyAction should have no DAI",async function(){
              let mpaAddress = deployedContracts.multiplyProxyActionsInstance.address;
              var balance =await  deployedContracts.daiTokenInstance.balanceOf(mpaAddress);
            ...
            it("should collect fee",async function(){
              var beneficiaryAfter =await  deployedContracts.daiTokenInstance.balanceOf("0x79d7176aE8F93A04bC73b9BC710d4b44f9e362Ce");
