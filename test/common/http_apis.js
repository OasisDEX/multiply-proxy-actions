...
const exchangeFromDAI = async function (
    ...
    sourceAmount.times(one.minus(fee)), precision).toFixed(0)}&fromAddress=${
    beneficiary}&slippage=${slippagePercentage.toNumber()}&disableEstimate=true&allowPartial=false`+
...
const exchangeToDAI = async function (
    ...
    var url = `https://api.1inch.exchange/v3.0/1/swap?fromTokenAddress=${fromTokenAddress}&toTokenAddress=${MAINNET_ADRESSES.MCD_DAI}&amount=${amountToWei(
      ...
      beneficiary
    }&slippage=${slippagePercentage.toNumber()}&disableEstimate=true&allowPartial=false` + (protocols ? `&protocols=${protocols}` : "");
    var _1inchResponse = await (await fetch(url)).json();
    var txData = _1inchResponse.tx;
    if (txData == undefined) {
        console.log("incorrect response from 1inch ", _1inchResponse, "original request", url);
